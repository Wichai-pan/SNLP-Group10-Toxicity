#!/bin/sh
# large-weighted: xlm-roberta-large + class-weighted loss (auto balanced)
# Tests whether large model + loss correction further reduces GER class bias
...
RUN_NAME="${RUN_NAME:-large-weighted}"
...
echo "=== large-weighted: xlm-roberta-large + class_weight auto ==="
echo "Train rows: $(wc -l < train.tsv)"
echo "Start: $(date)"
